...
pyg::Iterator<intv_type> py_intv_iterator(const c_dis_intvs &self) {
...
pyg::Iterator<intv_type> py_ovl_intv_iterator(const c_dis_intvs &self, const intv_type &key) {
...
pyg::Iterator<py_interval> py_ovl_item_iterator(const c_dis_intvs &self, const intv_type &key) {
    auto iter_pair = self.overlap_range(key);
    return pyg::make_iterator(iter_pair.first, iter_pair.second);
}
pyg::Iterator<py::object> py_ovl_val_iterator(const c_dis_intvs &self, const intv_type &key) {
    auto iter_pair = self.overlap_range(key);
...
get_first_overlap_item(const c_dis_intvs &self, const intv_type &key) {
    auto iter_pair = self.overlap_range(key);
